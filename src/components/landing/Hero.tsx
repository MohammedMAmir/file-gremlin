...
const Hero: React.FC<HeroProps> = ({ openSignIn: openSignIn, openSignUp: openSignUp }) => {
  ...
  async function handleMouseEnterRegister() {
    setRegisterIndex(Math.floor(Math.random() * gremlinTranslationsRegister.length));
  }
                ...
                {gremlinTranslationsRegister.map((item, i) => (
                  <div
                    ...
                    className={
                      ...
                      (registerIndex == i ? item + ' group-hover:opacity-100' : item)
                    }>
              ...
              <div className="group relative h-20" onMouseEnter={handleMouseEnterSign}>
                ...
                {gremlinTranslationsSign.map((item, i) => (
                  <div
                    ...
                    className={
                      `absolute inset-0 -z-10 h-10 w-10 opacity-0 transition-all duration-400 ease-in-out ` +
                      (signIndex == i ? item + ' group-hover:opacity-100' : item)
                    }>
                    <img className="object-fit" src="public/logo.svg"></img>
                  </div>
                ))}
